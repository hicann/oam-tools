...
setenv() {
    logandprint "[INFO]: Set the environment path."
...
# check the oam-tools module sub directory exist or not
...
logandprint "[INFO]: Begin upgrade oam-tools module."
logandprint "[INFO]: Uninstall the Existed oam-tools module before upgrade."
 ...
 installed in (${install_version_dir}) and then set the correct install path."
    ...
    if [ "$pkg_is_multi_version" = "true" ] && [ "${is_upgrade}" = "y" ]; then
        if [ -f "${_TARGET_INSTALL_PATH}/${upgrade_old_version_dir}/${ops_base_platform_old_dir}/script/uninstall.sh" ]; then
            ...
            if [ "$?" = 0 ]; then
                logandprint "[INFO]: Uninstall last oam-tools version module successfully."
            else
                logandprint "[ERROR]: Uninstall last oam-tools version module fail."
                exit 1
            fi
...
logandprint "[INFO]: Update the oam-tools install info."
...
# create copy and chmod oam-tools module path
logandprint "[INFO]: Install oam-tools module path in the install folder."
setenv
