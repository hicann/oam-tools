...
precleanbeforeinstall() {
    ...
    _files_existed=1
    # check the installation folder has files or oam-tools module existed or not
...
module, are you sure to keep installing oam-tools module in it? y"
        else
...
and the version of this package is $(getrunpkginfo "${KEY_RUNPKG_VERSION}"), do you want to continue? [y/n]"
            ...
            do
                read yn
                if [ "$yn" = n ]; then
                    logandprint "[INFO]: Exit to install oam-tools module."
                    exitlog
                    exit 0
                elif [ "$yn" = y ]; then
                    break;
        ...
        logandprint "[INFO]: Directory is empty, directly install oam-tools module."
    fi
...
Please install without quiet mode and check permission."
    ...
    if [ "${_files_existed}" = "0" ] && [ "${_installed_path}" = "${_path}" ]; then
        logandprint "[INFO]: Clean the installed oam-tools module before install."
...
path or clean the previous version oam-tools install info (/etc/ascend_install.info) and then reinstall it."
            return 1
        fi
...
check_opp_version_file () {
    ...
    elif [ -f "${_DEFAULT_INSTALL_PATH}/share/info/${opp_platform_dir}/version.info" ];then
        opp_ver_info="${_DEFAULT_INSTALL_PATH}/share/info/${opp_platform_dir}/version.info"
    else
...
show_relation () {
    ...
    if [ "$relation_situation" = "SUCC" ] ;then
        logandprint "[INFO]: Relationship of oam-tools with ${req_pkg_name_val} in path ${req_pkg_path} checked successfully"
    else
        logandprint "[WARNING]: Relationship of oam-tools with ${req_pkg_name_val} in path ${req_pkg_path} checked failed."
    fi
    return
}
...
interact_pre_check() {
    ...
    if [ "$?" != 0 ]; then
        if [ "${is_quiet}" = y ]; then
            logandprint "[WARNING]: Precheck of oam-tools module execute failed! do you want to continue install? y"
        else
            logandprint "[WARNING]: Precheck of oam-tools module execute failed! do you want to continue install?  [y/n] "
            while true
            do
            read yn
            if [ "$yn" = "n" ]; then
                echo "stop install oam-tools module!"
                exit 1
            elif [ "$yn" = y ]; then
                break;
            else
        ...
        if [ ${num_0} -lt 7 ] || [ ${num_1} -lt 5 ] || [ ${num_2} -lt 5 ]; then
            ...
            if [ "${is_quiet}" = y ]; then
                ...
                do
                    read yn
                    if [ "$yn" = n ]; then
                        logandprint "[INFO]: stop install oam-tools module!"
                        exitlog
                        exit 1
                    elif [ "$yn" = y ]; then
                        break;
...
logandprint "[INFO]: Execute the oam-tools run package."
...
if [ "${is_check}"="y" ] && [ "${check_path}" != "" ]; then
    ...
    if [ ! -f "${_VERCHECK_FILE}" ]; then
        logandprint "[ERROR]: ERR_NO:${FILE_NOT_EXIST};ERR_DES:The file\
(${_VERCHECK_FILE}) not exists. Please make sure that the oam-tools module\
...
if [ "${installed_version}" = "" ]; then
    logandprint "[INFO]: Version of installing oam-tools module is ${runpkg_version}."
else
    if [ "${runpkg_version}" != "" ]; then
        logandprint "[INFO]: Existed oam-tools module version is ${installed_version}, \
the new oam-tools module version is ${runpkg_version}."
    fi
fi
...
if [ "${is_check}" = "y" ]; then
    if [ -z "$pkg_version_dir" ]; then
        ...
        if [ $? -ne 0 ]; then
            logandprint "[ERROR]: ERR_NO:${OPP_COMPATIBILITY_CEHCK_ERR};ERR_DES:Check the compatibility of oam-tools package fail,please confirm the true version package."
            exit 1
	fi
    fi
fi
...
if [ "${is_install}" = "y" ];then
    ...
    # precheck before install oam-tools module
    ...
    if [ "${in_install_type}" != "devel" ]; then
        ...
        if [ "$?" != 0 ]; then
            logandprint "[ERROR]: ERR_NO:${INSTALL_FAILED};ERR_DES:Installation of oam-tools\
...
user group (${_DEFAULT_USERGROUP}) for devel mode? [y/n]"
    ...
    #getfirstnotexistdir "/usr/local/tmp/CANN-1.81/oam-tools"
    is_the_last_dir_opp=""
...
#            aicpuinfofile "add"
    ...
    if [ -z "$pkg_version_dir" ]; then
        ...
        if [ $? -ne 0 ]; then
            logandprint "[ERROR]: ERR_NO:${OPP_COMPATIBILITY_CEHCK_ERR};ERR_DES:Check the compatibility of oam_tools package fail,please confirm the true version package."
            exit 1
        fi
    fi
    # call opp_install.sh
    if [ -d "${target_dir}/oam_tools" ] && [ ! -L "${target_dir}/oam_tools" ] && [ -f "${target_dir}/oam_tools/script/uninstall.sh" ]; then
        bash "${target_dir}/oam_tools/script/uninstall.sh"
    fi
...
if [ "${is_upgrade}" = "y" ];then
    # if latest dir has no oam-tools module exit first!
    if [ "${is_for_all}" = y ] && [ $(id -u) -ne 0 ]; then
        check_dir_permission_for_common_user "${target_dir}"
    fi
    install_type=$(getinstalledinfo "${KEY_INSTALLED_TYPE}")
    # precheck before upgrade oam-tools module
    if [ "${is_precheck}" = "y" ];then
        interact_pre_check
    fi
    # check the compatibility of oam-tools
...
if [ "${is_uninstall}" = "y" ];then
    ...
    if [ ! -f "${_UNINSTALL_SHELL_FILE}" ]; then
        logandprint "[ERROR]: ERR_NO:${FILE_NOT_EXIST};ERR_DES:The file\
    (${_UNINSTALL_SHELL_FILE}) not exists. Please make sure that the oam-tools module\
