...
check_opp_version_file () {
    ...
    # oam-tools/version.info -> oam-tools
    elif [ -f "${_DEFAULT_INSTALL_PATH}/oam-tools/version.info" ];then
        ver_info="${_DEFAULT_INSTALL_PATH}/oam-tools/version.info"
    else
        logandprint "[ERROR]: ERR_NO:${FILE_NOT_EXIST}; The [oam-tools] version.info not exists."
    fi
    return
}
...
show_relation () {
    ...
    if [ "$relation_situation" = "SUCC" ] ;then
        logandprint "[INFO]: Relationship of oam-tools with ${req_pkg_name_val} in path ${req_pkg_path} check successfully"
        return 0
    else
        logandprint "[WARNING]: Relationship of oam-tools with ${req_pkg_name_val} in path ${req_pkg_path} check failed. \
