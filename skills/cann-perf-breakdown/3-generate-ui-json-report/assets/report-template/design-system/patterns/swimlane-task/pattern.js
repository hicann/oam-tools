...
(function registerPtoSwimlaneTaskPattern(global) {
  ...
  function drawTaskLabel(ctx, options) {
    const task = options.task || {};
    const text = String(task.label || task.displayName || task.rawName || 'compute');
    const barX = options.x || 0;
    const barY = options.y || 0;
    const width = Math.max(0, options.width || 0);
    const height = Math.max(0, options.height || 0);
    if (width < 28 || !text) return false;

    const radius = options.radius ?? 2;
    const fontFamily = options.fontFamily || 'sans-serif';
    const displayColor = resolveDisplayColor(options.baseColor || '#5f6775', {
      isSelected: options.isSelected,
      isEmphasized: options.isEmphasized,
    });
    const maxChars = Math.max(4, Math.floor((width - 8) / 6));
    const label = text.length > maxChars ? `${text.slice(0, Math.max(0, maxChars - 1))}…` : text;
    ...
    ctx.textAlign = 'left';
    ctx.textBaseline = 'middle';
    ctx.fillStyle = readableTextColor(displayColor);
    ctx.fillText(label, barX + 5, barY + height / 2 + 0.5);
    ctx.restore();
    return true;
  }
