...
def _estimate(rows, cycles_key, time_key, prefer_mix):
    ...
    for row in rows:
        ...
        if not cycles or not micros or cycles <= 0 or micros <= 0:
            continue
...
def main():
    ...
    try:
        ...
        sys.stderr.write(f"error: cannot read {args.details}: {exc}\n")
        return 1

    rows = payload.get("operators") if isinstance(payload, dict) else payload
    if not isinstance(rows, list):
        sys.stderr.write(f"error: no operator rows in {args.details}\n")
        return 1
    ...
    print(f"aicore_freq_mhz = {freq if freq is not None else 'unavailable'} "
