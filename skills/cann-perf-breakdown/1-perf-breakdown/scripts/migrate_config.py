...
def migrate(v1: dict) -> dict:
    ...
    layer_groups = []
    all_main_indices = set()
    for ltype, info in layer_types.items():
        idxs = info.get('layer_indices', []) or []
        all_main_indices.update(idxs)
        layer_groups.append({
            'type': ltype,
            'classification': 'unknown',
            'model_layer_indices': sorted(idxs),
            'source_ref': 'unknown',
        })

    num_main = len(all_main_indices) if all_main_indices else 'unknown'

    # trace instances: legacy layer_indices are ambiguous -> model_layer_index unknown
    trace_instances = []
    for ltype, info in layer_types.items():
        for inv, _idx in enumerate(info.get('layer_indices', []) or []):
            trace_instances.append({
                'instance_id': f'legacy_{ltype}_iter_{inv}',
                'model_layer_index': 'unknown',
                'invocation_index': inv,
                'layer_group_type': ltype,
            })
...
def main():
    ...
    if v1.get('schema_version') == 2:
        sys.stderr.write('输入已是 schema v2，无需迁移\n')
        sys.exit(1)
    ...
    with open(args.output, 'w', encoding='utf-8') as f:
        ...
        f.write('\n')
    print(f'已迁移为 v2 (legacy_unverified): {args.output}')
    print(f'  num_main_layers={v2["architecture"]["num_main_layers"]}  '
