...
def main():
    ...
    if not os.path.exists(args.raw_ops):
        sys.stderr.write(f'错误: 文件不存在: {args.raw_ops}\n')
        sys.exit(1)
    ...
    if not operators:
        sys.stderr.write('错误: raw_ops 无 operators\n')
        sys.exit(1)
    ...
    if not candidates:
        ...
        result = {
            ...
            'boundaries': best['positions'] if confidence >= args.min_confidence else [],
            'ranges': build_segments(operators, best) if confidence >= args.min_confidence else [],
            'candidates': [
                {
                    'kernel': c['kernel'],
                    'count': c['count'],
            ...
            'note': (
                ...
                + ('boundaries 可作 layer 候选起点，最终边界以源码语义为准。'
                   ...
                   else f'confidence < {args.min_confidence}，仅供参考，AI 应回退到全手动定位。')
            ),
        }
    ...
    print(f'op_segments 已保存到: {args.output}')
    print(f'  best_marker: {result["best_marker"]}')
    print(f'  confidence:  {result["confidence"]}')
    print(f'  layer count: {len(result["boundaries"])}')
