...
def _check_classification(manifest, counts, issues, detail):
    ...
    if moe_hits and classes and classes <= {'dense'}:
        ...
        issues.append({
            ...
            'repair_policy': {
                'owner_artifact': 'model_manifest',
                'repair_class': 'evidence_reconciliation',
                'allowed_targets': {
                    'analysis_config': ['$.architecture.layer_groups'],
                    'manifest_hypothesis': ['$.layer_groups'],
                },
                'trace_selectors': [{
                    'field': 'normalized_name',
                    'match': 'prefix',
                    'values': list(MOE_WITNESS_PREFIXES),
                }],
            },
...
def check_manifest_trace(manifest, raw_ops):
    ...
    # Divisibility: with N layers, a per-layer kernel must appear a whole multiple of N
    # times. MTP adds at most a couple of extra invocations, so allow a small remainder.
    inconsistent = {}
    for kernel, count in sorted(witnesses.items()):
        # A MoE-only witness spans the MoE subset, not the whole stack. Use that divisor when
        # the manifest declares it; fall back to num_main only when classification is absent,
        # where over-reporting is preferable to silently skipping the witness.
        is_moe_only = any(kernel.startswith(p) for p in MOE_ONLY_WITNESSES)
        ...
        if count % divisor == 0 and (count // divisor) in PLAUSIBLE_PER_LAYER:
            continue
        # tolerate a small MTP/warmup tail on top of a clean multiple
        salvaged = False
        for per_layer in PLAUSIBLE_PER_LAYER:
            excess = count - per_layer * divisor
            if 0 < excess <= 2 * per_layer:
                salvaged = True
                break
        if not salvaged:
            inconsistent[kernel] = {'count': count, 'per_layer_if_manifest': round(implied, 3),
                                    'divisor': divisor,
                                    'scope': 'moe_layers' if divisor == moe_layers else 'main_layers'}

    if inconsistent:
        # Report the counts, not a derived layer number. Witnesses cover different subsets
        # of the stack -- an attention anchor spans dense+MoE+MTP, a MoE gate only spans
        # MoE+MTP -- so a single N dividing all of them (their gcd) is an artefact, not a
        # layer count. Only when every witness agrees exactly is one invocation count
        # observable, and even that is invocations, not layers: one learned module called
        # N times is indistinguishable here from N modules called once.
        witness_text = ', '.join(f'{k}={v["count"]}' for k, v in sorted(inconsistent.items()))
        ...
        # Never an error, at any manifest confidence — bc.TRACE_CAN_ONLY_FALSIFY_COVERAGE.
        # The layer count is a scalar the source owns, and this check is not the coverage
        # direction: a trace covering one step witnesses only the paths that step executed
        # and its kernel counts are post-sharding, so it cannot arbitrate the count either
        # way. A divisibility mismatch is a report to the reader, not a verdict on either
        # side — the checkpoint config.json is what resolves it. The one direction the trace
        # CAN settle is coverage, and that is C1's job, not MT1's.
        confidence = bc.manifest_fact_confidence(manifest)
        severity = 'warning'
        ...
        issues.append({'id': 'MT1', 'severity': severity,
                       'node_path': 'model_manifest.num_main_layers',
                       'message': message})
...
def main():
    ...
    if args.output:
        with open(args.output, 'w', encoding='utf-8') as f:
            f.write(text + '\n')
        print(f'manifest/trace 交叉校验已写入: {args.output}  status={report["status"]}')
    else:
        print(text)
