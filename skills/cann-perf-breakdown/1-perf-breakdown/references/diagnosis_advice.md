...
- **advice**: `total_cost - kernel_sum` 占比高，说明成本主要在等待而非计算。先分类等待来源：按 `wait_time` 降序取本节点 top-5 kernel，看它们的前驱算子类型。前驱是 Hcom* 通信算子则属通信等待；前驱是计算算子而 wait 仍高则看是否跨流依赖；kernel_sum 极小而 wait 极大通常是 host 下发或图执行边界，不是设备侧问题。
