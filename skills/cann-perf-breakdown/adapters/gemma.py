...
"""
import ast
from .base import BaseAdapter, Fact
...
class GemmaAdapter(BaseAdapter):
    ...
    def refine(self, state):
        ...
        if 'sliding_window' in context.config_defaults:
            val, lineno = context.config_defaults['sliding_window']
            state.facts.append(Fact('sliding_window', val,
                                    context.ref(lineno), 'ast_default_arg', 'high'))
        ...
        state.gaps.append('Gemma sliding/full per-layer 分类由 config.__init__ 运行时构建，'
