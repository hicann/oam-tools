...
class TestCompileOp():
    ...
    def test_run_dirty_ub_tik_attribute_error(mocker, tmp_path):
        ...
        assert run_dirty_ub_tik({"compile_temp_dir": str(tmp_path / "x")}, "Ascend910B", 0) is False
    ...
    def test_get_compile_file_golden_have_temp():
        ...
        temp_dir.joinpath(ModeCustom.ADD_CUSTOM.value,
                          CompileOP.COMPILE_CHIP_MARKER).write_text("Ascend950", encoding='utf-8')
    ...
    def test_get_compile_file_other_chip_not_reuse(mocker):
        ...
        temp_dir.joinpath(ModeCustom.ADD_CUSTOM.value,
                          CompileOP.COMPILE_CHIP_MARKER).write_text("Ascend950", encoding='utf-8')
        compile_file_path.joinpath(
            f'{ModeCustom.ADD_CUSTOM.value}_add_custom.o').write_text("old", encoding='utf-8')
        mocker.patch("shutil.which", return_value=False)
