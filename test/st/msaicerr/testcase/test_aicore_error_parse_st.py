...
class TestUtilsMethods(CommonAssert):
    ...
    def common_mock(mocker):
        ...
        mocker.patch.object(aicore_error_parser.Path, "write_text", return_value=None)
    ...
    def test_parse(self, mocker, collect_path, ffts_flag, parse_level, result):
        ...
        mocker.patch.object(AicoreErrorParser, "_need_atomic_clean", return_value=False)
        mocker.patch.object(
            AicoreErrorParser, "write_tiling_data_to_file", return_value=False
        )
        mocker.patch.object(AicoreErrorParser, "run_test_env", return_value=True)
        mocker.patch.object(AicoreErrorParser, "comment_cce_in_case", return_value=1)
        mocker.patch.object(
            AicoreErrorParser,
            "_test_single_op",
            return_value=[RetCode.SUCCESS, "11", "single_op"],
        )
        mocker.patch.object(
            AicoreErrorParser, "collect_driver_aicore_number", return_value=1
        )
    ...
    def test_decompile(self, mocker, tmp_path):
        ...
        info.cce_file = os.path.join(kernel_meta_path, f"{info.kernel_name}.cce")
        info.aic_error_info["current_pc"] = "x1435"
        info.aic_error_info["start_pc"] = "x0123"
        parser = AicoreErrorParser(os.path.join(RES_PATH, "ori_data/collect/l1/"))
        parser.parse_level = 1
    ...
    def test_test_single_op(self, mocker):
        ...
        mocker.patch(
            "subprocess.run",
            return_value=mocker.MagicMock(returncode=0, stdout=b"", stderr=b""),
        )
    ...
    def test_test_single_op_subprocess_failed(self, mocker):
        # 子进程非 0 退出且日志无 aicore error：用例没真正跑起来，按 NOT_RUN 上报。
        mocker.patch(
            "subprocess.run",
    ...
    def test_get_op_info(self, mocker):
        ...
        self.assertIn(msg, "6. Execution Result of the Single-Operator Test Case")
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (True, "Dump data tid is not the same with rts tid."),
    ...
    def test_get_parse_info_error(self, mocker, plog_file, log_res):
        ...
        if "block_dim" in log_res:
            mocker.patch.object(
                parser, "get_dump_data_info", return_value=[("1592077", "data_name")]
            )
        else:
            mocker.patch.object(
                parser, "get_dump_data_info", return_value=[("1", "data_name")]
            )
