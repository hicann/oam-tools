...
REPORT_PATH = os.path.join(os.path.dirname(__file__),
                           '../res/ori_data/2020-12-24-01-13-00/')
REPORT_PATH_APPLOG = os.path.join(os.path.dirname(__file__),
                                  '../res/ori_data/2020-12-24-01-13-01/')
REPORT_PATH_NO_AICERR = os.path.join(os.path.dirname(__file__),
                                     '../res/ori_data/2020-12-24-01-13-02/')
...
MILAN_DATA_PATH = os.path.join(os.path.dirname(
    __file__), '../res/ori_data/collect_milan')
...
class TestUtilsMethods(CommonAssert):
    ...
    def test_no_cce(self, mocker):
        args = ['msaicerr.py', '-p',
                get_case_path('../res/ori_data/no_cce/asys_aicerror')]
        mocker.patch('sys.argv', args)
        mocker.patch(
            'ms_interface.collection.Collection.check_dump_data_is_valid', return_value=None)
        mocker.patch(
            'ms_interface.collection.Collection.check_host_and_device_kernel_name', return_value=True)
        mocker.patch(
            'ms_interface.aicore_error_parser.AicoreErrorParser._decompile')
        mocker.patch('shutil.which', return_value='/home/cce-objdump')
        mocker.patch(
            'ms_interface.aicore_error_parser.AicoreErrorParser._get_data_dump_result', return_value=False)
        mocker.patch('msaicerr.verify_device_id', return_value=True)
        mocker.patch('ms_interface.aicore_error_parser.AicoreErrorParser.collect_driver_aicore_number',
                     return_value=24)
        mocker.patch(
            'ms_interface.aicore_error_parser.AicoreErrorParser.run_test_env', return_value=True)
        mock_stdout = mocker.patch("sys.stdout", new_callable=StringIO)
        msaicerr.main()
        screen_print = mock_stdout.getvalue()
