...
def test_bundle_branch_initialized_in_checkopts():
    ...
    build_content = get_build_script_content()
    checkopts = re.search(r"^checkopts\(\) \{\n(?P<body>.*?)^\}",
                          build_content, re.S | re.M)
...
def _run_resolve(tmp_path, *, explicit=None, refs=(), with_git=True):
    ...
    for ref in refs:
        ...
        subprocess.run([GIT, "update-ref", f"refs/remotes/{ref}", "HEAD"],
                       cwd=repo, check=True)
    ...
    (proj / "CMakeLists.txt").write_text(textwrap.dedent(f"""
...
def test_resolve_falls_back_without_git(tmp_path):
    ...
    assert _run_resolve(tmp_path, refs=("origin/9.1.0-beta.2",), with_git=False) == "master"
...
def _run_install_with_existing_bundle(tmp_path, *, present_branch, explicit):
    ...
    return subprocess.run(
        [CMAKE, "-S", str(proj), "-B", str(tmp_path / "build"),
...
def test_existing_bundle_dir_branch_is_verified():
    ...
    assert "oam_resolve_bundle_branch" in body, \
        "复用已有 bundle 前应先解析目标分支"
...
def test_download_libs_whitelist_matches_cmake():
    ...
    cmake_match = re.search(r"set\(OAM_BUNDLE_KNOWN_BRANCHES (?P<v>[^)]*)\)", cmake_content)
    assert cmake_match
