...
BIU_ERROR = "0x0\nbiu_err_addr bit[24:0]=0000000000000000000000000  in hex:0x0"
...
CUBE_ERR_INFO = (
    "CUBE_ERR_INFO: No CUBE_ERR_INFO found\n"
...
CCU_ERR_INFO = (
    "CCU_ERR_INFO : No CCU_ERR_INFO found\n"
    "ccu_illegal_instr, 非法执行: 1.指令的binary错误  "
    "2.指令地址非对齐"
)

BIU_ERR_INFO = (
    "BIU_ERR_INFO : No BIU_ERR_INFO found\n"
...
class TestUtilsMethods(CommonAssert):
    def test_find_extra_pc(self, mocker):
        ...
        mocker.patch(
            "ms_interface.utils.hexstr_to_list_bin", return_value=[63, 62, 61, 60]
        )
    ...
    def test_tiling_data(self, mocker):
        ...
        self.assertIn(
            res,
            "tiling data in float16: [-0.10980224609375, 1.1513671875, 874.5, 2.384185791015625e-07]",
        )
    ...
    def test_single_op_exec_succ(self):
        aicerr_info = AicErrorInfo()
        aicerr_info.single_op_test_result = RetCode.SUCCESS
        res = aicerr_info.get_conclusion()
        self.assertEqual(True, "the AI Core error was not reproduced" in res)

    def test_single_op_not_run_attempted(self):
        # 单算子测试尝试过（attempted=True）且结果为 NOT_RUN 时，输出具体排查建议
        aicerr_info = AicErrorInfo()
        aicerr_info.single_op_attempted = True
        aicerr_info.single_op_test_result = RetCode.NOT_RUN
        res = aicerr_info.get_conclusion()
        self.assertEqual(
            True,
            "Check whether the *.o and *.json files of the operator are complete"
            in res,
        )

    def test_single_op_not_run_not_attempted(self):
        # 单算子测试未尝试（attempted=False，如 SK/dump 失败/采集早退），
        # NOT_RUN 初值不应误触 NOT_RUN 分支，应回落内部错误兜底分支
