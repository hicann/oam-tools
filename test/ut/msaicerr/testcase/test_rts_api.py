...
class TestClassAscendOpKernelRunner:
    ...
    @pytest.mark.parametrize("rt_error", [ctypes.c_uint64(0), ctypes.c_uint64(1), 0x07000000])
    @staticmethod
    ...
    def test_memcpy(self, mocker, rts_api):
        ...
        assert rts_api.memcpy(ctypes.c_void_p(
            0), 10, ctypes.c_void_p(0), 10) is None

    def test_memcpy_failed(self, mocker, rts_api):
        mocker.patch.object(rts_api, 'parse_error', side_effect=RuntimeError('test'))
        mocker.patch.object(rts_api.rtsdll, 'rtMemcpy', return_value=0)
        try:
            rts_api.memcpy(ctypes.c_void_p(0), 1, b'0x00', 1)
        except RuntimeError:
            assert True
        else:
            assert False
    ...
    def test_dll_simulator_so(self, mocker, rts_api):
        ...
        with pytest.raises(RuntimeError):
            ...
            getattr(rts_api, "_dll_simulator_so")(
                ['so_path1', 'so_path1'], 'ld_lib_paths', 'simulator_dump_path')
        ...
        mocker.patch('ctypes.CDLL', return_value=Mock())
        assert getattr(rts_api, "_dll_simulator_so")(
            ['so_path1', 'so_path1'], 'ld_lib_paths', 'simulator_dump_path') is None
    ...
    def test_load_simulator_so(self, mocker, rts_api):
        ...
        mocker.patch('os.path.exists', return_value=True)
        mocker.patch.object(rts_api, '_init_simulator_so_path',
    ...
    def test_register_device_binary_kernel(self, mocker, rts_api):
        mocker.patch.object(
            rts_api.rtsdll, 'rtDevBinaryRegister', return_value=0)
        mocker.patch(
            'ms_interface.single_op_test_frame.utils.file_util.read_file', return_value=b'0x00')
        res = rts_api.register_device_binary_kernel('kernel_path', '')
        assert isinstance(res, ctypes.c_void_p)

    def test_unregister_device_binary_kernel(self, mocker, rts_api):
        mocker.patch.object(
            rts_api.rtsdll, 'rtDevBinaryUnRegister', return_value=0)
        ptr = ctypes.c_void_p(0)
    ...
    def test_register_function(self, mocker, rts_api):
        mocker.patch.object(
            rts_api.rtsdll, 'rtFunctionRegister', return_value=0)
    ...
    def test_copy_bin_file_to_hbm(self, mocker, rts_api):
        with pytest.raises(TypeError):
            mocker.patch(
                'ms_interface.single_op_test_frame.utils.file_util.read_file', return_value='0x00')
    ...
    def test_api_call(self, mocker, rts_api):
        mocker.patch.object(rts_api.rtsdll, 'rtMemset', return_value=0)
        assert rts_api.api_call(
            'rtMemset', ctypes.c_void_p(0), 1, 1, 1) is None
    ...
    def test_launch_kernel(self, mocker, rts_api):
        ...
        assert rts_api.launch_kernel(ctypes.c_void_p(
            0), 1, (), 1, 0, ctypes.c_uint64(0)) == 0

        mocker.patch.object(rts_api.rtsdll, 'rtKernelLaunch',
    ...
    def test_synchronize_with_stream(self, mocker, rts_api):
        mocker.patch.object(
            rts_api.rtsdll, 'rtStreamSynchronize', return_value=0)
        assert rts_api.synchronize_with_stream(ctypes.c_uint64(0)) == 0

        mocker.patch.object(rts_api.rtsdll, 'rtStreamSynchronize',
                            return_value=ctypes.c_uint64(0))
        assert rts_api.synchronize_with_stream(ctypes.c_uint64(0)) == 0
    ...
    def test_start_online_profing(self, mocker, rts_api):
        mocker.patch.object(
            rts_api.rtsdll, 'rtStartOnlineProf', return_value=0)
    ...
    def test_get_online_profiling_data(self, mocker, rts_api):
        mocker.patch.object(
            rts_api.rtsdll, 'rtGetOnlineProfData', return_value=0)
