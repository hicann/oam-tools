...
class TestUtilsMethods(CommonAssert):
    ...
    def test_parse_dump_data_error(self, mocker, capsys):
        ...
        mocker.patch.object(dump_data_parser, "_save_data_to_bin_file", side_effect=ValueError("test"))
    ...
    def test_summary_tensor_without_dtype_bfloat16(self):
        dump_data_parser = DumpDataParser(dump_file, AicErrorInfo())
        res = getattr(dump_data_parser, "_summary_tensor_without_dtype")('text.bin', 'bfloat16')
    ...
    def test_summary_tensor_empty_array(self, mocker):
        ...
        res = getattr(dump_data_parser, "_summary_tensor_without_dtype")("text.bin", "float32")
        self.assertIn(res, "Max: N/A, Min: N/A, Mean: N/A, Std: N/A")

    def test_check_tensor_data_type_error(self):
        dump_data_parser = DumpDataParser(dump_file, AicErrorInfo())
        res = getattr(dump_data_parser, "_check_tensor_data")('input', 1, np.array([1, 2]), 'bfloat112')
    ...
    def test_check_tensor_data_type_inf_error(self):
        dump_data_parser = DumpDataParser(dump_file, AicErrorInfo())
        res = getattr(dump_data_parser, "_check_tensor_data")('input', 1, np.array([np.inf, 2]), 'float16')
        print(res)
        self.assertIn(res, 'input[1] NaN/INF. Input data invalid. Please check!')

    def test_check_tensor_data_type_gt_max_error(self):
        dump_data_parser = DumpDataParser(dump_file, AicErrorInfo())
        res = getattr(dump_data_parser, "_check_tensor_data")('input', 1, np.array([59000, 2]), 'float16')
    ...
    def test_to_numpy_dtype(self):
        ...
        self.assertEqual(getattr(DumpDataParser, "_to_numpy_dtype")('float32'), np.dtype('float32'))
    ...
    def test_to_numpy_dtype_bfloat16_with_ext(self, mocker):
        ...
        self.assertEqual(getattr(DumpDataParser, "_to_numpy_dtype")('bfloat16'), bf16_dtype)
    ...
    def test_get_item_dtype(self):
        ...
        # 枚举不在映射表中，直接记录枚举值
        self.assertEqual(get_item_dtype({'data_type': 77}, 'input', {'input': {}}, 0), '77')
        # 枚举不在映射表但json有dtype，优先json
        self.assertEqual(
            get_item_dtype({'data_type': 77}, 'input', {'input': {0: 'float16'}}, 0), 'float16')
        # undefined不作为有效dtype，回退json
        self.assertEqual(
            get_item_dtype({'data_type': 0}, 'input', {'input': {0: 'int32'}}, 0), 'int32')
        self.assertEqual(
    ...
    def test_build_typed_array(self):
        ...
        raw = np.arange(6, dtype=np.float32).tobytes()
        array, np_dtype = getattr(DumpDataParser, "_build_typed_array")(raw, 'float32', [2, 3])
        self.assertEqual(np_dtype, np.dtype('float32'))
        self.assertEqual(array.shape, (2, 3))
        # 非numpy dtype
        array, np_dtype = getattr(DumpDataParser, "_build_typed_array")(raw, 'int4', [6])
        self.assertEqual(np_dtype, None)
        self.assertEqual(str(array.dtype), 'int8')
        # 字节数未对齐
        array, np_dtype = getattr(DumpDataParser, "_build_typed_array")(b'\x01\x02\x03', 'float32', [1])
        self.assertEqual(np_dtype, None)
        # data为None时按空数组处理
        array, np_dtype = getattr(DumpDataParser, "_build_typed_array")(None, 'float32', [1])
        self.assertEqual(array.size, 0)

    def test_summary_tensor_array(self):
        """已知dtype的数组直接由内存计算summary"""
        res = getattr(DumpDataParser, "_summary_tensor_array")(np.array([1, 3], dtype=np.int32), 'int32')
        self.assertIn(res, 'If dtype is int32, summary is: Max: 3, Min: 1, Mean: 2.0, Std: 1.0')

    def test_summary_tensor_array_empty(self):
        res = getattr(DumpDataParser, "_summary_tensor_array")(np.array([], dtype=np.float32), 'float32')
        self.assertIn(res, 'Max: N/A, Min: N/A, Mean: N/A, Std: N/A')
    ...
    def test_summary_tensor_array_error(self, mocker):
        ...
        res = getattr(DumpDataParser, "_summary_tensor_array")(np.array([1, 2], dtype=np.int32), 'int32')
    ...
    def test_build_dst_file_name_normal_len(self):
        ...
        res = build_dst_file_name(str(self.temp), 'input', 0, 'float32', np.dtype('float32'))
    ...
    def test_build_dst_file_name_oversize_bin_keeps_suffix(self):
        ...
        info.kernel_name = 'a' * 260
        dump_data_parser = DumpDataParser(str(self.temp), info)
        res = getattr(dump_data_parser, "_build_dst_file_name")(str(self.temp), 'input', 0, 'hifloat8', None)
    ...
    def test_gen_random_numeric_name(self, mocker):
        ...
        res = getattr(dump_data_parser, "_gen_random_numeric_name")(str(self.temp), '.npy')
        self.assertEqual(res, '2222222222222222.npy')

    def test_record_mapping_append(self):
        """多条映射追加写入同一份mapping.csv"""
        dump_data_parser = DumpDataParser(str(self.temp), AicErrorInfo())
        getattr(dump_data_parser, "_record_mapping")(str(self.temp), '1111111111111111.npy', 'long_name_a.npy')
        getattr(dump_data_parser, "_record_mapping")(str(self.temp), '2222222222222222.bin', 'long_name_b.bin')
        mapping_text = self.temp.joinpath(Constant.MAPPING_CSV_FILE).read_text(encoding='utf-8')
    ...
    def test_load_name_mapping(self):
        ...
        self.assertEqual(getattr(dump_data_parser, "_load_name_mapping")(), {'long_name_a': '1234567890123456'})
