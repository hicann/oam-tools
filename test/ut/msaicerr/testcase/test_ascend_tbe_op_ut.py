...
def make_mock_device():
    ...
    return device
SAMPLE_JSON = {
    'blockDim': 8, 'kernelName': 'test_kernel',
    'magic': 'RT_DEV_BINARY_MAGIC_ELF', 'parameters': [None],
    'workspace': {'size': [128]}, 'opParaSize': 64,
}
...
def test_init_bad_profiling_times_type(mocker):
    mocker.patch.object(AscendOpKernelRunner, 'get_rts_api', return_value=make_mock_device())
...
def test_fill_inputs_with_np(mocker):
    runner, _ = make_runner(mocker)
    kernel_args, input_params = ([], [])
    getattr(runner, '_fill_inputs')([np.ones(4, dtype=np.int8)], kernel_args, input_params, 'magic')
    assert len(kernel_args) == 1
...
def test_fill_workspace_with_param(mocker):
    runner, _ = make_runner(mocker)
    kernel = make_kernel_obj(workspace=[128], parameters=[{'dtype': 'float16', 'init_value': 1}])
    kernel_args, wksp = ([], [])
    getattr(runner, '_fill_workspace')(kernel, 0, wksp, kernel_args, 'magic')
    assert len(wksp) == 1
...
def test_fill_outputs_normal(mocker):
    ...
    getattr(runner, '_fill_outputs')(kernel, [], out_info, [], output_params, kernel_args, 'magic')
...
def test_fill_outputs_with_ref(mocker):
    ...
    out_info = [{'shape': [4], 'dtype': 'float16'}]
    output_params, kernel_args = ([], [])
    getattr(runner, '_fill_outputs')(kernel, [[0, 0]], out_info, [in_param], output_params, kernel_args, 'magic')
    assert len(output_params) == 1
...
def test_fill_outputs_skip_none(mocker):
    ...
    kernel = make_kernel_obj()
    output_params, kernel_args = ([], [])
    getattr(runner, '_fill_outputs')(kernel, [], [None], [], output_params, kernel_args, 'magic')
...
def test_check_magic_false(mocker):
    runner, device = make_runner(mocker)
    magic_bytes = (np.ones(AscendOpKernel.MagicMemorySize, dtype=np.int8) * AscendOpKernel.MagicData).tobytes()
...
def test_execute_kernel_register_fallback(mocker):
    ...
    kernel = make_kernel_obj(stub_func_name='op')
    kernel.is_registered_to_device.return_value = False
    device.register_function.side_effect = [RuntimeError('fail'), ctypes.c_void_p(16384)]
...
def test_run_failed_ret(mocker):
    runner, _ = make_runner(mocker)
    mocker.patch.object(runner, 'exec_single_case', side_effect=[[None, [1, 0, 0]], [None, [0, 0, 2]]])
    param = _exec_param()
    result = runner.run(param)
