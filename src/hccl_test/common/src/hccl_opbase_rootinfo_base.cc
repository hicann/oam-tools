...
int HcclOpBaseTest::init_buf_val() { return 0; }

int HcclOpBaseTest::check_buf_result() { return 0; }
...
void HcclOpBaseTest::is_data_overflow()
{
    if (op_type == HCCL_REDUCE_PROD) {
        ...
        if ((dtype == HCCL_DATA_TYPE_INT8 || dtype == HCCL_DATA_TYPE_UINT8) && rank_size >= RANKSIZE_TH_INT8) {
            no_verification();
        }
        if ((dtype == HCCL_DATA_TYPE_INT32 || dtype == HCCL_DATA_TYPE_UINT32) && rank_size >= RANKSIZE_TH_INT32) {
            no_verification();
        }
        if ((dtype == HCCL_DATA_TYPE_INT64 || dtype == HCCL_DATA_TYPE_UINT64) && rank_size >= RANKSIZE_TH_INT64) {
            no_verification();
        }
    } else if (op_type == HCCL_REDUCE_SUM) {
        if ((dtype == HCCL_DATA_TYPE_INT8 || dtype == HCCL_DATA_TYPE_UINT8) && rank_size >= RANKSIZE_TH_INT64) {
            no_verification();
        }
    }
...
void HcclOpBaseTest::print_execution_time(double average_time_us, double algorithm_bandwith_GBytes_s)
{
    ...
    if (check_err != 0) {
        ...
        printf(
            "local rank_id %d, check result failed, %-17llu | %-14.2f | %-20.5f | failed\n", rank_id, data->data_size,
    ...
    if (rank_id == root_rank) {
        ...
        if (!check_result[root_rank]) {
            printf(
                "%-17llu | %-14.2f | %-20.5f | failed\n", data->data_size, average_time_us,
                algorithm_bandwith_GBytes_s);
        } else {
            printf(
                "%-17llu | %-14.2f | %-20.5f | success\n", data->data_size, average_time_us,
                algorithm_bandwith_GBytes_s);
        }
    }
    return;
