...
#include <math.h>
namespace hccl {
#define REDUCESCATTERV_SUM_MAX_RANKSIZE(HcclDataType) \
    int64_t(sqrt(((int64_t(1) << int(HcclDataTypePrecision::CONCAT(HcclDataType, ))) - 1)))
#define REDUCESCATTERV_SUM_RESULE_OVERFLOW(rank_size, HcclDataType) \
    bool(int64_t(rank_size) > REDUCESCATTERV_SUM_MAX_RANKSIZE(HcclDataType))
...
private:
    ...
    virtual int
    check_buf_result(); // （recv_buf拷贝到recvbufftemp,并且校验正确性）需要调用check_buf_init，校验正确性要调用check_buf_result_float
