...
size_t HcclOpBaseReduceTest::init_malloc_Ksize_by_data() { return data->count * data->type_size; }
...
int HcclOpBaseReduceTest::hccl_op_base_test() // 主函数
{
    ...
    for (int j = 0; j < warmup_iters; ++j) {
        HCCLCHECK(HcclReduce(
            (void*)send_buff, (void*)recv_buff, data->count, (HcclDataType)dtype, (HcclReduceOp)op_type, root_rank,
            hccl_comm, stream));
    }
