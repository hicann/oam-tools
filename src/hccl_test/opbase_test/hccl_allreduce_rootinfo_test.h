...
namespace hccl {
#define ALLREDUCE_SUM_MAX_RANKSIZE(HcclDataType) \
    int64_t(((int64_t(1) << int(HcclDataTypePrecision::CONCAT(HcclDataType, ))) - 1) / 2)
#define ALLREDUCE_SUM_RESULE_OVERFLOW(rank_size, HcclDataType) \
    bool(int64_t(rank_size) > ALLREDUCE_SUM_MAX_RANKSIZE(HcclDataType))
