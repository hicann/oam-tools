...
#include <math.h>
namespace hccl {
#define REDUCESCATTER_SUM_MAX_RANKSIZE(HcclDataType) \
    int64_t(sqrt(((int64_t(1) << int(HcclDataTypePrecision::CONCAT(HcclDataType, ))) - 1)))
#define REDUCESCATTER_SUM_RESULE_OVERFLOW(rank_size, HcclDataType) \
    bool(int64_t(rank_size) > REDUCESCATTER_SUM_MAX_RANKSIZE(HcclDataType))
