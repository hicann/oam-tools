...
void HcclOpBaseReducescatterTest::is_data_overflow()
{
    if (op_type == HCCL_REDUCE_PROD) {
        ...
        if (dtype == HCCL_DATA_TYPE_INT8 && REDUCESCATTER_SUM_RESULE_OVERFLOW(rank_size, HCCL_DATA_TYPE_INT8)) {
            ...
            dtype == HCCL_DATA_TYPE_INT16 && REDUCESCATTER_SUM_RESULE_OVERFLOW(rank_size, HCCL_DATA_TYPE_INT16)) {
            no_verification();
        } else if (
            dtype == HCCL_DATA_TYPE_INT32 && REDUCESCATTER_SUM_RESULE_OVERFLOW(rank_size, HCCL_DATA_TYPE_INT32)) {
            no_verification();
        } else if (dtype == HCCL_DATA_TYPE_FP16 && REDUCESCATTER_SUM_RESULE_OVERFLOW(rank_size, HCCL_DATA_TYPE_FP16)) {
            no_verification();
        } else if (
            dtype == HCCL_DATA_TYPE_BFP16 && REDUCESCATTER_SUM_RESULE_OVERFLOW(rank_size, HCCL_DATA_TYPE_BFP16)) {
            no_verification();
