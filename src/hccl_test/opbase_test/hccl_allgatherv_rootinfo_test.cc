...
int HcclOpBaseAllgatherVTest::check_buf_result()
{
    ...
    ACLCHECK(aclrtMemcpy(
        (void*)recv_buff_temp, malloc_kSize * rank_size, (void*)recv_buff, malloc_kSize * rank_size,
...
int HcclOpBaseAllgatherVTest::hccl_op_base_test() // 主函数
{
    ...
    for (int j = 0; j < warmup_iters; ++j) {
        HCCLCHECK(HcclAllGatherV(
            (void*)send_buff, data->count, (void*)recv_buff, recv_counts, recv_disp, (HcclDataType)dtype, hccl_comm,
            stream));
    }
