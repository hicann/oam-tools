...
namespace hccl {
HcclOpBaseAlltoallTest::HcclOpBaseAlltoallTest() : HcclOpBaseTest(), sendCount_(0), recvCount_(0) {}
...
int HcclOpBaseAlltoallTest::hccl_op_base_test() // 主函数
{
    ...
    for (int j = 0; j < warmup_iters; ++j) {
        HCCLCHECK(HcclAlltoAll(
            (void*)send_buff, sendCount_, (HcclDataType)dtype, (void*)recv_buff, recvCount_, (HcclDataType)dtype,
            hccl_comm, stream));
    }
