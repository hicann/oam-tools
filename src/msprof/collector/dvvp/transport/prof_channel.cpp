...
{
    MSPROF_EVENT("device id %d, channel: %d, total_size_channel: %lld bytes, file:%s, job_id:%s,"
                 "channelReadCnt:%lld, readExecCnt: %u, dispatchInCnt: %u, dispatchOutCnt: %u, "
                 "uploadDataMaxDuration: %llu.",
                 deviceId_, static_cast<int32_t>(channelId_), totalSize_, relativeFileName_.c_str(),
