...
{
    MSPROF_EVENT(
        "device id %d, channel: %d, total_size_channel: %lld bytes, file:%s, job_id:%s,"
        "channelReadCnt:%lld, readExecCnt: %u, dispatchInCnt: %u, dispatchOutCnt: %u, "
        "uploadDataMaxDuration: %llu.",
        deviceId_, static_cast<int32_t>(channelId_), totalSize_, relativeFileName_.c_str(), jobCtx_->job_id.c_str(),
        drvChannelReadCont_, readExecCnt_, totalSchedulingInCnt_, totalSchedulingOutCnt_, uploadDataMaxDuration_);
    std::string tag =
    ...
    do {
        ...
        if (currLen <= 0) {
            if (currLen < 0) {
                ...
                MSPROF_INNER_ERROR(
                    "EK9999", "read device %d, channel:%d, ret=%d", deviceId_, static_cast<int32_t>(channelId_),
                    currLen);
            }
...
void ChannelReader::UploadData()
{
    ...
    if (UploaderMgr::instance()->UploadData(jobCtx_->job_id, fileChunkReq) == PROFILING_FAILED) {
...
void ChannelReader::FlushDrvBuff()
{
    ...
    if (ret != PROFILING_SUCCESS) {
        ...
        MSPROF_INNER_ERROR(
            "EK9999", "DrvProfFlush failed, deviceId:%d, channelId:%d,ret:%d", deviceId_, channelId_, ret);
        guard.unlock();
        return;
    }
