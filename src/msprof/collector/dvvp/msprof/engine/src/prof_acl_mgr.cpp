...
ProfAclMgr::ProfAclMgr() : isReady_(false), isProfWarmup_(false), mode_(WORK_MODE_OFF), curDevId_(-1), devSet_({}),
...
void ProfAclMgr::ProfDataTypeConfigHandle(SHARED_PTR_ALIA<analysis::dvvp::message::ProfileParams> params)
{
    ...
    if (params->geApi == MSVP_PROF_L0) {
        dataTypeConfig_ |= PROF_FWK_SCHEDULE_L0;
    }
    if (params->geApi == MSVP_PROF_L1) {
        dataTypeConfig_ |= PROF_FWK_SCHEDULE_L0 | PROF_FWK_SCHEDULE_L1;
    }
...
int32_t ProfAclMgr::ProfStartCommon(const uint32_t *devIdList, uint32_t devNums)
{
    ...
    if (!GetDevicesNotify(devIdList, devNums)) {
        return MSPROF_ERROR_UNINITIALIZE;
    }
    // check device online
    std::string info;
    for (uint32_t i = 0; i < devNums; i++) {
        if (!(ProfManager::instance()->CheckIfDevicesOnline(std::to_string(devIdList[i]), info))) {
            MSPROF_LOGE("DevId:%u is not online, error info:%s", devIdList[i], info.c_str());
            MSPROF_INNER_ERROR("EK9999", "DevId:%u is not online, error info:%s", devIdList[i], info.c_str());
            return MSPROF_ERROR;
        }
    }
    ...
    // start device tasks
    for (uint32_t i = 0; i < devNums; i++) {
        if (devIdList[i] == DEFAULT_HOST_ID) {
            continue;
        }
        if (MsprofDeviceHandle(devIdList[i]) != PROFILING_SUCCESS) {
            continue;
        }
        ...
        uint32_t startDevIdList[1] = {devIdList[i]};
        ...
        if (ret != ACL_SUCCESS) {
            ...
            MSPROF_INNER_ERROR("EK9999", "CommandHandleProfStart failed, devId:%u", devIdList[i]);
            return MSPROF_ERROR;
        }
    }
...
bool ProfAclMgr::GetDevicesNotify(const uint32_t *deviceId, uint32_t devNums)
{
    std::lock_guard<std::mutex> lk(mtx_);
    for (uint32_t i = 0; i < devNums; i++) {
