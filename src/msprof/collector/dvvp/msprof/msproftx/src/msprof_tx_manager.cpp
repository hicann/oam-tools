...
int32_t MsprofTxManager::MarkExPoint(aclrtStream stream, MsprofTxInfo &info)
{
    ...
    uint32_t currWriteCusor = 0;
    uint32_t nextWriteCusor = 0;
    uint32_t cycles = 0;
    do {
        cycles++;
        FUNRET_CHECK_EXPR_ACTION(cycles >= MARKEX_MAX_CYCLE, return PROFILING_FAILED, "MarkEx index cycle overflow.");
        currWriteCusor = markExIndex_.load(std::memory_order_relaxed);
        nextWriteCusor = currWriteCusor + 1;
    } while (!markExIndex_.compare_exchange_weak(currWriteCusor, nextWriteCusor));
    const int32_t ret = rtProfilerTraceExFunc_(currWriteCusor, MARKEX_MODEL_ID, MARKEX_TAG_ID,
        static_cast<void *>(stream));
    FUNRET_CHECK_EXPR_ACTION(ret != PROFILING_SUCCESS, return PROFILING_FAILED,
        "[MarkEx]Failed to call rtProfilerTraceEx, mark index: %u.", currWriteCusor);
    info.value.stampInfo.markId = currWriteCusor;
    MSPROF_LOGI("[MarkEx]Success to mark ex point in device, index: %u.", currWriteCusor);
    return PROFILING_SUCCESS;
}
