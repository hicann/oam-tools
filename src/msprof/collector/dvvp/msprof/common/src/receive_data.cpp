...
{
    MSPROF_LOGI("start pop api_event data");
...
{
    MSPROF_LOGI("start pop compact data");
...
{
    MSPROF_LOGI("start pop additional data");
    // data binding with tag and ageFlag
    ...
    for (; batchSizeMax < MSVP_BATCH_MAX_LEN;) {
        ...
        if (!isOK) {
            MSPROF_LOGI("additional info data type is:%d", data.type);
            break;
        }
        totalPopCount_++;
...
{
    MSPROF_LOGI("start pop variable additional data");
    uint64_t allStartTime = analysis::dvvp::common::utils::Utils::GetClockMonotonicRaw();
    size_t outSize = 262144;
    ...
    uint64_t allEndTime = analysis::dvvp::common::utils::Utils::GetClockMonotonicRaw();
    MSPROF_LOGD("Variable Addinfo handle data successfully, dump cost: %llu ns, all cost: %llu ns, data size: %zu",
        (dumpEndTime - dumpStartTime), (allEndTime - allStartTime), outSize);
}
