...
struct FeatureRecord {
        ...
        std::string tempAffectedComponent, std::string tempAffectedComponentVersion, std::string tempInfoLog) noexcept
    {
        ...
        FUNRET_CHECK_EXPR_PRINT(
            ret != EOK, "Failed to copy affectedComponentVersion %s to struct and ret is %d", tempFeatureName.c_str(),
            ret);
...
public:
    ...
    int32_t Init();
    void Uninit();
    FeatureRecord* GetIncompatibleFeatures(size_t* featuresSize) const;
