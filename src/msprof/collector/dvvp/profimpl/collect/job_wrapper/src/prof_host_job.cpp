...
int32_t ProfHostCcaMsJob::Init(const SHARED_PTR_ALIA<CollectionJobCfg> cfg)
{
    ...
    if (collectionJobCfg_->comParams->params->host_numa_profiling.compare(MSVP_PROF_ON) == 0 ||
        collectionJobCfg_->comParams->params->host_threads_sync_profiling.compare(MSVP_PROF_ON) == 0 ||
        collectionJobCfg_->comParams->params->host_cache_profiling.compare(MSVP_PROF_ON) == 0) {
        return PROFILING_SUCCESS;
    } else {
...
int32_t ProfHostService::Uninit()
{
    ...
    if (hostTimerTag_ == PROF_HOST_SYS_DISK) {
        ...
        if (OsalChmod(fileName.c_str(), FILE_MODE) != OSAL_EN_OK) {
            in.close();
...
void ProfHostService::StoreData() const
{
    ...
    if (fileChunk->chunkSize != 0) {
        if (analysis::dvvp::transport::UploaderMgr::instance()->UploadData(params->job_id, fileChunk) !=
            ...
            MSPROF_INNER_ERROR(
                "EK9999", "Upload host server data failed , jobId: %s, fileName: %s", params->job_id.c_str(),
                uploadFileName.c_str());
        }
    }
...
int32_t ProfHostService::CollectToolIsRun()
{
    ...
    if (!(Utils::IsFileExist(redirectionPath))) {
        MSPROF_LOGE("The file:%s does not exist", redirectionPath.c_str());
        MSPROF_INNER_ERROR("EK9999", "The file:%s does not exist", redirectionPath.c_str());
        return PROFILING_FAILED;
    }
