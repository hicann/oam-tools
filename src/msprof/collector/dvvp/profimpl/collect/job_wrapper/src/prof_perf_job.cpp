...
void PerfExtraTask::StoreData(const std::string& fileName)
{
    ...
    if (len <= 0 || len > MSVP_LARGE_FILE_MAX_LEN) {
        MSPROF_LOGE("Invalid data file size: %lld bytes, valid range: (0, %lld] bytes.", len, MSVP_LARGE_FILE_MAX_LEN);
        MSPROF_INNER_ERROR(
            "EK9999", "Invalid data file size: %lld bytes, valid range: (0, %lld] bytes.", len,
            MSVP_LARGE_FILE_MAX_LEN);
        return;
    }
