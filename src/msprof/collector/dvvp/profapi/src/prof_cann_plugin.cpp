...
void ProfCannPlugin::ProfInitReportBuf(uint32_t type)
{
    ...
    variableAdditionalBuffer_.Init("variable", additionalBufferSize);
    ...
    ProfRegisterFunc(REPORT_VARIABLE_ADDITIONAL_INDEX_SHIFT, reinterpret_cast<VOID_PTR>(TryIndexShiftAdprofBuf));
}
 ...
 * @return MSPROF_ERROR_UNINITIALIZE
           MSPROF_ERROR_NONE
 */
int32_t ProfCannPlugin::ProfReportAdditionalInfo(uint32_t agingFlag, const VOID_PTR data, uint32_t len)
{
    if (len > STANDARD_ADDITIONAL_INFO_LENGTH) {
        const MsprofShapeInfo *info = static_cast<const MsprofShapeInfo *>(data);
        return variableAdditionalBuffer_.BatchPush(info, len);
    } else {
        MsprofAdditionalInfo *info = static_cast<MsprofAdditionalInfo *>(data);
        if (info->level == MSPROF_REPORT_AICPU_LEVEL) {
            return batchAdditionalBuffer_.BatchPush(info, len);
        } else {
            // fix 0 to 256B
            if (len < STANDARD_ADDITIONAL_INFO_LENGTH) {
                size_t zeroLen = ADDITIONAL_INFO_BODY_LENGTH - sizeof(info->data);
                errno_t ret = memset_s(info->data + len - 24, zeroLen, 0, zeroLen);
