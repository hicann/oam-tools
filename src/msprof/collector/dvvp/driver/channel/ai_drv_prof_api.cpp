...
int32_t DrvSocPmuTaskStart(int32_t profDeviceId, AI_DRV_CHANNEL profChannel, std::string& multiSocPmuEvents)
{
    ...
    for (uint32_t i = 0; i < static_cast<uint32_t>(socPmuEventList.size()); ++i) {
        if (configPos >= configSize) {
            MSPROF_LOGW(
                "Unable to pack soc pmu param, config pos: %zu bytes, config size: %zu bytes.", configPos, configSize);
            break;
        }
...
void DrvCopySocPmuSmmuDFXParam(void* configP, size_t configSize, size_t& configPos)
{
    ...
    if ((configPos + cfgSize) > configSize) {
        MSPROF_LOGW(
            "Soc pmu smmu dfx param overflow, configSize: %zu bytes, configPos: %zu bytes.", configSize, configPos);
...
int32_t DrvBiuPerfStop(int32_t profDeviceId, AI_DRV_CHANNEL profChannel)
{
    MSPROF_EVENT(
        ...
        static_cast<int32_t>(profChannel));
    int32_t ret = prof_stop(static_cast<uint32_t>(profDeviceId), profChannel);
    if (ret == PROF_OK) {
        MSPROF_EVENT(
...
int32_t DrvProfFlush(uint32_t deviceId, uint32_t channelId, uint32_t& bufSize)
{
    ...
    if (ret == DRV_ERROR_NONE) {
        ...
        MSPROF_CALL_ERROR(
            "EK9999", "Failed to halProfDataFlush.deviceId=%u, channelId=%u, ret=%d", deviceId, channelId, ret);
        return PROFILING_FAILED;
    }
#endif
