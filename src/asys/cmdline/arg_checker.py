...
def ill_char_check(arg_name, arg_val):
    ...
    if not rex:
        log_error(f"Argument {arg_name} is invalid, only characters in [a-zA-Z0-9_-.], input {arg_val}")
...
def check_arg_create_dir(arg_name, arg_val):
    ...
    if os.path.exists(arg_val):
        ...
        if not os.access(arg_val, os.W_OK):
            log_error("Argument \"{0}\" is not permissible to write, argument value: \"{1}\"".
                      format(arg_name, arg_val))
        ...
        if ret != RetCode.SUCCESS:
            log_error("Argument \"{0}\" is not permissible to create, argument value: \"{1}\"".
                      format(arg_name, arg_val))
        return ret
...
def check_arg_device_id(arg_name, arg_val):
    ...
    if arg_val >= devices_num:
        log_error("'-d' value should be in [0, {}), input {}".format(devices_num, arg_val))
        return RetCode.FAILED
    return RetCode.SUCCESS
