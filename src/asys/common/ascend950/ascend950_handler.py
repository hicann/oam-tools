...
class Ascend950Handler(DeviceInfo):
    ...
    def get_device_bus_info(self, device_id):
        ...
        bus_v0, ring_f0, cpu_f0, mate_f0, l2_buf_f0 = super().get_device_bus_info(
            device_id
        )
        component_one_id = self.get_encode_component_one_id(device_id)
        bus_v1, ring_f1, mate_f1, l2_buf_f1 = [
            NOT_SUPPORT,
            NOT_SUPPORT,
            NOT_SUPPORT,
            NOT_SUPPORT,
        ...
        if component_one_id:
            bus_v1, ring_f1, _, mate_f1, l2_buf_f1 = super().get_device_bus_info(
                component_one_id
            )
        return [
            f"{bus_v0}, {bus_v1}",
    ...
    def get_device_hbm_info(self, device_id):
        ...
        try:
            ret = self.dsmi_handle.dsmi_get_device_info(
                ...
                p_memory_info,
                ctypes.pointer(
                    ctypes.c_uint(ctypes.sizeof(DsmiNormalMemoryInfoStru()))
                ),
            )
    ...
    def get_device_aicore_frequency(self, device_id):
        ...
        aicore_frequency = super().get_device_aicore_frequency(device_id)
        return (
            f"{aicore_frequency}(Avg)"
            if aicore_frequency != NOT_SUPPORT
            else NOT_SUPPORT
        )
    ...
    def get_device_temperature(self, device_id):
        ...
        try:
            ret = self.dsmi_handle.dsmi_get_soc_sensor_info(
                ctypes.c_int32(device_id), SOC_TEMP_ID, p_temperature
            )
        except AttributeError:
            return NOT_SUPPORT
