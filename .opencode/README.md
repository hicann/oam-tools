# oam-tools 仓 Agent skills 规划

## oam-tools 仓 skills 清单
...
- [ ] **oam-tools-reviewer** — 遵循各种编码规范、编码军规、模块软件设计约束检视代码
- [ ] **gitcode-pipeline** — 触发流水线任务、查询流水线状态、获取失败任务日志
- [ ] **oam-tools-dt-runner** — 编译和执行 UT/ST 用例
- [ ] **oam-tools-tester** — 生成用例，在带有npu的环境上执行用例
...
- 执行测试用例或样例：使用oam-tools-tester在真实环境中执行用例

## oam-tools 仓 skills 路径

- 项目组共享，希望能做到启动agent默认安装或更新
- 仅在oam-tools仓使用的skills可直接提交到oam-tools仓`.claude/skills`目录
- 多个仓共用的skills，源码在公共仓（当前是https://gitcode.com/cann-agent/skills），启动agent时会自动下载或者更新skills到`.claude/skills`目录

> **注意**：`gitcode-issue`、`gitcode-pr` 等跨仓共享的 skills 不在 oam-tools 仓的 `.claude/skills` 目录中维护。它们的源码存放在公共仓，在 oam-tools 目录下启动 agent 后会自动下载并安装到本地 `.claude/skills` 目录。
...
flowchart TB
    ...
    subgraph 编码验证阶段
        D1[编写代码]
        D2[执行ut/st]
        D3[oam-tools-reviewer<br/>agent检视代码]
        D4[oam-tools-tester<br/>本地验证]
    end
