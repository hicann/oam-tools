...
- **./bin/\<executable_file\>**：必选，集合通信性能测试工具的执行命令。
  ...
  > [!NOTE]说明
  > 各测试命令在不同产品上的支持情况，以对应通信算子接口的实际支持能力为准，详细可参见[通信算子接口](https://gitcode.com/cann/hccl/blob/9.2.0-beta.2/docs/zh/api_ref/comm_op_interface/README.md)。
...
- **-d \<datatype\>或--datatype \<datatype\>**：可选，HCCL Test支持的数据类型，默认值为fp32。
  ...
  > [!NOTE]说明
  > 各测试命令在不同产品上支持的数据类型，以对应通信算子接口的实际支持能力为准，详细可参见[通信算子接口](https://gitcode.com/cann/hccl/blob/9.2.0-beta.2/docs/zh/api_ref/comm_op_interface/README.md)。
...
- **-t 或 --onlydevicetime**：可选，将通信算子在Host侧的软件耗时与kernel加载耗时排除在通信执行耗时之外，仅统计Device侧的执行时间（即影响HCCL Test执行耗时的关键部分）。
